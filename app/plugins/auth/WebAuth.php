...
class WebAuth
{
	...
	public static function register($user, $pass, $id, $domain)
	{
		...
		$key    = base64_encode(self::_gen_auth_key($id, $user, $pass, $domain));
	...
	public static function login(pixelpost\Request $request) 
	{
		try
		{
			...
			$auth = self::_get_auth($user, $pass, $request->get_host())
				 ->set_challenge($priv)
				 ->set_lifetime(150);
	...
	public static function forget(pixelpost\Request $request)
	{
		try
		{
			...
			$content = pixelpost\Template::create()
				->assign('user', $user)
	...
	public static function reset(pixelpost\Request $request)
	{
		try
		{
			...
			try
			{
				if (!$request->is_post())
				{
					pixelpost\Template::create()->publish('auth/tpl/reset.php');
					return;
				}
				else
				{
