...
class Request
{
    ...
    public function auto()
    {
        ...
        $serverPort = isset($_SERVER['HTTP_HOST'])   ? $_SERVER['HTTP_HOST']   : 80; // SERVER_PORT send incorrect value
    ...
    public function set_url($url)
    {
        ...
        if (isset($parse['scheme']))   $this->_scheme = $parse['scheme'];
        if (isset($parse['host']))     $this->_host   = $parse['host'];
        if (isset($parse['port']))     $this->_port   = $parse['port'];
        if (isset($parse['user']))     $this->_user   = $parse['user'];
        if (isset($parse['pass']))     $this->_pass   = $parse['pass'];
        if (isset($parse['fragment'])) $this->_frag   = $parse['fragment'];
        if (isset($parse['path']))     $this->_path   = ltrim($parse['path'], '/');
        if (isset($parse['query']))    parse_str($parse['query'], $query);
    ...
    public function set_query(array $params)
    {
        ...
        {
            $this->_query = array();
            foreach($params as $i => $v) $this->_query[$i] = stripslashes($v);
        }
        else
        {
            $this->_query = $params;
    ...
    public function set_method($method)
    {
        ...
        switch ($method)
        {
            case self::METHOD_GET     : $this->_method = 'GET';     break;
            case self::METHOD_POST    : $this->_method = 'POST';    break;
            case self::METHOD_PUT     : $this->_method = 'PUT';     break;
            case self::METHOD_HEAD    : $this->_method = 'HEAD';    break;
            case self::METHOD_DELETE  : $this->_method = 'DELETE';  break;
            case self::METHOD_TRACE   : $this->_method = 'TRACE';   break;
            case self::METHOD_CONNECT : $this->_method = 'CONNECT'; break;
            case self::METHOD_OPTIONS : $this->_method = 'OPTIONS'; break;
    ...
    public function get_base_url()
    {
        ...
        if ( ! $this->is_std_port() ) $url .= ':' . $this->_port;
        if (   $this->is_userdir()  ) $url .= '/' . $this->_userdir;
    ...
    public function get_request_url()
    {
        ...
        if ( $this->is_path() )        $url .= $this->get_path();
        if ( $this->_isTrailingSlash ) $url .= '/';
        if ( $this->is_query() )       $url .= '?' . http_build_query($this->_query);
        if ( $this->is_fragment() )    $url .= '#' . $this->get_fragment();
